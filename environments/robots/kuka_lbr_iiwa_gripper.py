...
class Kuka_iiwa_gripper(gym.Env):
    def __init__(self, bullet_client, offset=(0, 0, 0), sim_time=0., scale=1.,
        ...
        self.robot = bullet_client.loadSDF("robots/kuka_lbr_iiwa/kuka_with_gripper2.sdf", 0)
        ...
        self.bullet_client.setJointMotorControlArray(self.robot[0],
                                                     self.joints_fingers,
                                                     p.VELOCITY_CONTROL,
    ...
    def reset(self, desired_state=None):
        ...
        if desired_state is not None:
            ...
            for joint_id, joint in self.joints.items():
                ...
                self.bullet_client.resetJointState(self.robot[0], joint_id,
                                                   joint_position)

            self.bullet_client.stepSimulation()
            contact_points = self.bullet_client.getContactPoints(self.robot[0],
                                                                 self.robot[0])
    ...
    def step(self, action: np.ndarray):
        ...
        for joint_id, action_joint in zip(self.joints_arm, action_arm):
            ...
            position, _, _, _ = self.bullet_client.getJointState(self.robot[0], joint_id)
    ...
    def get_observation(self):
        ...
        tcp_position, _, _, _, _, _, tcp_velocity, _ = \
            self.bullet_client.getLinkState(self.robot[0], 8,
...
if __name__ == "__main__":
    ...
    robot = Kuka_iiwa_gripper(p, sim_time=.1, scale=.1)
